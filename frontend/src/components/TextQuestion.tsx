...
export default function TextQuestion({ type, value, onChange, placeholder, error }: TextQuestionProps) {
  ...
  return (
    <div className="mt-1">
      <textarea
        className={`w-full border rounded-md px-3 py-2 text-sm md:text-base focus:outline-none focus:ring-1 ${
          error ? 'border-red-500 focus:ring-red-200' : 'border-[#DADCE0]'
        }`}
      ...
      <div className="mt-1 flex flex-col items-end">
        <span className={`text-sm ${reached ? 'text-[#FF6200]' : 'text-gray-500'}`}>
          {value.length}/{limit}
        </span>
        {reached && (
